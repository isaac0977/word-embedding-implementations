...
def eval_wordsim_bert(model, tokenizer, f='data/wordsim353/combined.tab'):
    ...
    for line in open(f, 'r').readlines()[1:]:
        ...
        w2 = splits[1] 
        sim.append(float(splits[2]))
        v1 = bert.generate_average_word_embedding(w1, model, tokenizer).cpu().detach().numpy()
        v2 = bert.generate_average_word_embedding(w2, model, tokenizer).cpu().detach().numpy()
        pred.append(np.dot(v1, v2)/(np.linalg.norm(v1) * np.linalg.norm(v2)))
...
def build_vocab_bats_bert(matrix, vocab, indices, model, tokenizer, f):
    pairs = [line.strip().split() for line in open(f, 'r').readlines()]
    ...
    for pair in pairs:
        ...
        if word not in vocab and len(word) > 0:
            vocab_builder_helper(word, matrix, vocab, indices, model, tokenizer)
...
def vocab_builder_helper(word, matrix, vocab, indices, model, tokenizer):
    index = len(vocab)
    indices[word] = index
    vocab.append(word)
    matrix.append(bert.generate_average_word_embedding(word, model, tokenizer).cpu().detach().numpy())

def eval_bats_file_bert(matrix, vocab, indices, model, tokenizer, f, repeat=False):
    '''
        Evaluates a trained embedding model on a single BATS file using either
        3CosAdd (the classic vector offset cosine method) or 3CosAvg (held-out
        averaging).
    ...
    for i in range(len(pairs)):
        ...
        if ("_" in pairs[j][0]):
            pairs[j][0].replace("_", " ")
        a = bert.generate_average_word_embedding(pairs[i][0], model, tokenizer).cpu().detach().numpy()
        c = bert.generate_average_word_embedding(pairs[j][0], model, tokenizer).cpu().detach().numpy()
        for bw in pairs[i][1]:
            if ("_" in bw):
                bw.replace("_", " ")
            qa.append(a)
            qb.append(bert.generate_average_word_embedding(bw, model, tokenizer).cpu().detach().numpy())
            qc.append(c)
            groups.append(i)
            targets.append([pair.replace("_", " ") for pair in pairs[j][1]])
            exclude.append([pairs[i][0], bw, pairs[j][0]])
        ...
        for i in range(len(exclude)):
            for w in exclude[i]:
                if(len(w) > 0):
                    sims[i][indices[w.replace("_", " ")]] = 0

    preds = [vocab[np.argmax(x)].replace("_", " ") for x in sims]
    accs = [1 if preds[i].lower() in targets[i] else 0 for i in range(len(preds))]
    regrouped = np.zeros(np.max(groups) + 1)
    for a, g in zip(accs, groups):
        regrouped[g] = max(a, regrouped[g])
...
def eval_bats_bert(model, tokenizer):
    '''
        Evaluates a trained embedding model on BATS.
    ...
    for dr in os.listdir('data/BATS'):
        if os.path.isdir(os.path.join(base, dr)):
            #Build the static embedding list using the Vocab from BATS
            for f in os.listdir(os.path.join(base, dr)):
                matrix, vocab, indices = build_vocab_bats_bert(matrix, vocab, indices, model, tokenizer, os.path.join(base, dr, f))
    for dr in os.listdir('data/BATS'):
        if os.path.isdir(os.path.join(base, dr)):
            dk = dr.split('_', 1)[1].lower()
            accs[dk] = []
            for f in os.listdir(os.path.join(base, dr)):
                print(f)
                accs[f.split('.')[0]] = eval_bats_file_bert(matrix, vocab, indices, model, tokenizer, os.path.join(base, dr, f))        
                accs[dk].append(accs[f.split('.')[0]])
            accs[dk] = [a for a in accs[dk] if a is not None]
            accs[dk] = np.mean(accs[dk]) if len(accs[dk]) > 0 else None
...
def eval_msr_bert(model, tokenizer):
    '''
        Evaluates a trained embedding model on the MSR paraphrase task using
        logistic regression over cosine similarity scores.
    '''
    ...
    for ss in X_tr:
        train = []
        train.append(bert.generate_sentence_embeddings(' '.join(word for word in ss[0]),model, tokenizer).cpu().detach().numpy())
        train.append(bert.generate_sentence_embeddings(' '.join(word for word in ss[1]),model, tokenizer).cpu().detach().numpy())
        tr_cos.append(1 - cosine(train[0], train[1]))
        count += 1

        if (count % 100 == 0):
            print('training sentence # ', count)
    lr = LogisticRegression(class_weight='balanced', solver='liblinear')
    lr.fit(np.array(tr_cos).reshape(-1,1), y_tr)

    print("train complete")
    for ss in X_test:
        test = []
        test.append(bert.generate_sentence_embeddings(' '.join(word for word in ss[0]),model, tokenizer).cpu().detach().numpy())
        test.append(bert.generate_sentence_embeddings(' '.join(word for word in ss[1]),model, tokenizer).cpu().detach().numpy())
        test_cos.append(1 - cosine(test[0], test[1]))
    preds = lr.predict(np.array(test_cos).reshape(-1, 1))
...
if __name__ == "__main__":
    ...
    if (args.path == "BERT"):
        tokenizer = bert.load_tokenizer()
        model = bert.load_model()
        
        #print('[evaluate] WordSim353 correlation:')
        #ws = eval_wordsim_bert(model, tokenizer)
        #print(ws)

        print('[evaluate] BATS accuracies:')
        bats = eval_bats_bert(model, tokenizer)
        print(bats)


        #print('[evaluate] MSR accuracy:')
        #msr = eval_msr_bert(model, tokenizer)
        #print(msr)
    else:
